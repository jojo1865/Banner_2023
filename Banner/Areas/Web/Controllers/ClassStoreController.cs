...
namespace Banner.Areas.Web.Controllers
{
    public class ClassStoreController : PublicClass
    {
                    ...
                    if (PT != null)
                    {
                        ...
                        if (OH_ != null)
                        {
                            ...
                            switch (PT.PayTypeID)
                            {
                                ...
                                case 3://PayPal
                                    SetAlert("", 1, "/Web/ClassStore/Order_Paid_PayPal/" + OH.OHID);
                                    break;
        ...
        public cOrder_Paid_Credit_Card GetOrder_Paid_PayPal(int OHID)
        {
            string OrderInfo = "";
            ...
            else
            {
                ...
                {
                    OP = DC.Order_Paid.FirstOrDefault(q => q.OHID == OHID && q.PayType.PayTypeID == 3);
                    if (OP == null)
                        Error = "此訂單並非選擇以PayPal付款";
                    else if (OP.PaidFlag)
                        Error = "此訂單已完成付款,請至您的訂單檢視";
                }
                ...
                else
                {
                    ...
                    TempData["PayPalID"] = OP.PayType.MerchantID;
        ...
        [HttpGet]
        public ActionResult Order_Paid_PayPal(int ID)
        {
            GetViewBag();
            return View(GetOrder_Paid_PayPal(ID));
        }
        #endregion
        #region Paypal返回
        [HttpGet]
        public ActionResult Order_Back_PayPal()
        {
            GetViewBag();
            Order_Header OH = null;
            int OHID = GetQueryStringInInt("OHID");
            int Success = GetQueryStringInInt("Type");
            int OPID = 0;
            ACID = GetACID();
            OH = DC.Order_Header.FirstOrDefault(q => q.OHID == OHID);
            if (OH != null)
            {
                ...
                var OP = DC.Order_Paid.FirstOrDefault(q => q.OHID == OHID && q.PayType.PayTypeID == 3 && !q.PaidFlag);
                if (OP != null)
                {
                    OPID = OP.OPID;
                    if (Success == 1)//付款完成
